...
else if (isset($_POST['form_sent']))
{
	...
	switch ($section)
	{
		...
		case 'personal':
		{
            ...
            elseif ($form['birthd']=='0' or $form['birthm']=='0' or $form['birthy']=='0') {
              $form['birthdate'] = NULL;
            } else {
              $form['birthdate'] = $form['birthd'].'/'.$form['birthm'].'/'.$form['birthy'];
            }
            unset($form['birthd']);
            unset($form['birthm']);
            unset($form['birthy']);
    ...
    $user['g_moderator'] == '1'))))                                                             // or the user is another mod
{
    ...
    if ($user['sex'] != '')
    {
        $user['sex'] = str_replace("0", "<img src=\"img/male.gif\" alt=\"".$lang_common['Male']."\" /> ".$lang_common['Male'], $user['sex']);
        $user['sex'] = str_replace("1", "<img src=\"img/female.gif\" alt=\"".$lang_common['Female']."\" /> ".$lang_common['Female'], $user['sex']);
        $user['sex'] = str_replace("2", "<img src=\"img/donut.gif\" alt=\"".$lang_common['Bigg']."\" /> ".$lang_common['Bigg'], $user['sex']);
...
<?php
            ...
            foreach ($styles as $temp)
            {
                ...
                if ( $temp != 'OnEnAGros-v5' )
                    echo "\t\t\t\t\t\t\t\t".'<option value="'.$temp.'"'.$selected.'>'.str_replace('_', ' ', $temp).'</option>'."\n";
                else if ( $temp == 'OnEnAGros-v5' && $pun_user['is_admmod'] )
                    echo "\t\t\t\t\t\t\t\t".'<option value="'.$temp.'"'.$selected.'>'.str_replace('_', ' ', $temp).'</option>'."\n";
            }
