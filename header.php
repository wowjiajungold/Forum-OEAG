...
else
{
    ...
    {
        // Boxes status
        $mp_boxes_full = ($pun_user['num_pms'] >= $pun_user['g_pm_limit']) ? true : false;
        $mp_boxes_empty = ($pun_user['num_pms'] <= '0') ? true : false;
        if ($pun_user['g_pm_limit'] != '0' && !$pun_user['is_admmod'])
        {   
            if ($mp_boxes_empty)
                $tpl_temp .= "\n\t\t\t\t".'<li><span>'.$lang_pms['Empty boxes'].'</span></li>';
            elseif ($mp_boxes_full)
                $tpl_temp .= "\n\t\t\t\t".'<li><span><a href="pms_inbox.php"><strong>'.$lang_pms['Full boxes'].'</strong></a></span></li>';
            else
            {
                $per_cent_box = ceil($pun_user['num_pms'] / $pun_user['g_pm_limit'] * '100');
                $tpl_temp .= "\n\t\t\t\t".'<li><span>'.sprintf($lang_pms['Full to'],$per_cent_box.'%').' <div id="mp_bar_ext"><div id="mp_bar_int" style="width:'.$per_cent_box.'px;"><!-- --></div></div></span></li>';
            }
        }
        ...
        if ($num_new_mp > '0')
            $tpl_temp .= "\n\t\t\t\t".'<li><span><a href="pms_inbox.php"><strong>'.($num_new_mp == '1' ? $lang_pms['New message'] : sprintf($lang_pms['New messages'],$num_new_mp)).'</strong></a></span></li>';        
    }

	if (in_array(basename($_SERVER['PHP_SELF']), array('index.php', 'search.php')))
		$tpl_temp .= "\n\t\t\t".'</ul><br />'."\n\t\t\t".'<ul class="conr">'.($pun_user['g_search'] == '1' ? "\n\t\t\t\t".'<li><span><a href="search.php?action=show_new">'.$lang_common['Show new posts'].'</a></span></li>' : '')."\t\t\t\t\t".'<li><span><a href="search.php?action=show_24h">'.$lang_common['Show recent posts'].'</a></span></li>'."\n\t\t\t\t".'<li><span><a href="misc.php?action=markread">'.$lang_common['Mark all as read'].'</a></span></li>'."\n\t\t\t".'</ul>'."\n\t\t\t".'<div class="clearer"></div>'."\n\t\t".'</div>';
