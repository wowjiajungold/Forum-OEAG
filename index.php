...
if ($pun_config['o_users_online'] == '1')
{
    ...
    $result = $db->query('SELECT count(id) FROM '.$db->prefix.'users WHERE last_visit >= \''.$todaystamp.'\'') or error('Impossible de retrouver la liste des utilisateurs en ligne aujourd\'hui', __FILE__, __LINE__, $db->error());
    $users_today = $db->fetch_row($result);
    ...
    {
        echo "\t\t\t".'<dl id="birthdaylist" class="clearb">'."\n\t\t\t\t".'<dt><strong>'.$lang_index['Birthday'].' </strong></dt>'."\n";
        foreach($birthdays as $birthday)
        {
            echo "\t\t\t\t".'<dd><a href="profile.php?id='.$birthday[0].'">'.pun_htmlspecialchars($birthday[1]).'</a> ('.calculAge($birthday[2]).')</dd>';
        }
        echo "\n\t\t\t".'</dl>'."\n";
    }
    else
