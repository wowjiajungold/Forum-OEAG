...
if (isset($_POST['form_sent']))
{
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
        ...
        if(!isset($_POST['silent']) && (time() - $cur_post['posted']) < 2 * $pun_user['g_double_post'] * 60)
        {
            $db->query('UPDATE '.$db->prefix.'topics SET last_post='.$now.', last_post_id='.$id.', last_poster=\''.$db->escape($pun_user['username']).'\' WHERE id='.$cur_post['tid']) or error('Unable to update topic', __FILE__, __LINE__, $db->error());
            $db->query('UPDATE '.$db->prefix.'forums SET last_post='.$now.', last_post_id='.$id.', last_poster=\''.$db->escape($pun_user['username']).'\' WHERE id='.$cur_post['fid']) or error('Unable to update topic', __FILE__, __LINE__, $db->error());
...
<?php endif; ?>						<label class="required"><strong><?php echo $lang_common['Message'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br />
						<textarea name="req_message" id="req_message" rows="20" cols="95" tabindex="<?php echo $cur_index++ ?>"><?php echo pun_htmlspecialchars(isset($_POST['req_message']) ? $message : $cur_post['message']) ?></textarea><br /></label>
<?php /* FluxToolBar */
