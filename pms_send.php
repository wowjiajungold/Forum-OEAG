...
if (isset($_POST['form_sent'])) // The post button has been pressed
{
	...
	foreach ($dest_list as $destinataire)
	{
		...
		if ($destinataires[$i] = $db->fetch_assoc($result_username))
		{
			...
			// Are we authorized?
			elseif (!$pun_user['is_admmod'] && $destinataires[$i]['allow_msg'] !== null && $destinataires[$i]['allow_msg'] == '0')
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		if (empty($r) && empty($edit)) // It's a new message
		{
			...
			foreach ($destinataires as $dest)
			{
				...
				{
					$mail_message = str_replace('<pm_url>', $pun_config['o_base_url'].'/pms_view.php?tid='.$shared_id.'&mid='.$new_mp, $mail_message);
		...
		if (!empty($r)) // It's a reply or a reply with a quote
		{
			...
			foreach ($destinataires as $dest)
			{
				...
				if ($dest['id'] == $pun_user['id'])
					...
					if ($pun_config['o_pms_notification'] == '1' && $dest['notify_pm'] == '1' && $dest['id'] != $pun_user['id'])
					{
						$mail_message = str_replace('<pm_url>', $pun_config['o_base_url'].'/pms_view.php?tid='.$r.'&mid='.$new_mp, $mail_message);
						pun_mail($dest['email'], $mail_subject, $mail_message);
					}
			}
			$db->query('UPDATE '.$db->prefix.'users SET last_post='.$now.' WHERE id='.$pun_user['id']) or error('Unable to update user', __FILE__, __LINE__, $db->error());
...
<?php
// Fetch contacts
$result = $db->query('SELECT contact_name FROM '.$db->prefix.'contacts WHERE user_id='.$pun_user['id'].' ORDER BY contact_name ASC') or error('Unable to update to find the list of the contacts', __FILE__, __LINE__, $db->error());

if ($db->num_rows($result))
{
?>
				<label><a href="pms_contacts.php"><?php echo $lang_pms['Contacts'] ?></a><br />
				<select name="p_contact">
					<option value="0"><?php echo $lang_pms['Others'] ?></option>
<?php
	while ($cur_contact = $db->fetch_assoc($result))
		echo "\t\t\t\t\t".'<option value="'.pun_htmlspecialchars($cur_contact['contact_name']).'"'.($p_contact == $cur_contact['contact_name'] ? ' selected="selected"' : '').'>'.pun_htmlspecialchars($cur_contact['contact_name']).'</option>'."\n";
?>
				</select>
				<br /></label>
<?php
	...
	echo '<p><a href="pms_contacts.php">'.$lang_pms['No contacts'].'</a></p>';
?>
				</div>
				<div class="clearer"></div>
				<p><?php echo $lang_pms['Send multiple'].$pun_config['o_pms_max_receiver'].'.'?></p>
