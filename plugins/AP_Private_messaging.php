...
if (isset($_POST['form_sent']))
{
	...
	{
		$db->query('UPDATE '.$db->prefix.'groups SET g_pm='.$set.' WHERE g_id=\''.$id.'\'') or error('Unable to change the permissions', __FILE__, __LINE__, $db->error());
	}
	
	while (list($id, $set) = @each($limit))
	{
		$db->query('UPDATE '.$db->prefix.'groups SET g_pm_limit='.intval($set).' WHERE g_id=\''.$id.'\'') or error('Unable to change the permissions', __FILE__, __LINE__, $db->error());
	}
